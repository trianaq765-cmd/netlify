...
function getPlayerID(event) { return event.headers['x-player-id'] || null; }

async function logAccess(event, action, success, details = {}) {
    const log = { ip: getClientIP(event), hwid: getHWID(event), playerId: getPlayerID(event), ua: event.headers['user-agent']?.substring(0, 80) || 'unknown', action, success, path: event.path, ts: new Date().toISOString(), ...details };
    await db.addLog(log);
}
...
function json(statusCode, body) {
    return { statusCode, headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Methods': 'GET,POST,DELETE,OPTIONS', 'Access-Control-Allow-Headers': 'Content-Type,x-admin-key,x-hwid,x-player-id,x-executor-token,x-roblox-id,x-place-id,x-job-id' }, body: JSON.stringify(body) };
}

function text(statusCode, body, contentType = 'text/plain') {
    return { statusCode, headers: { 'Content-Type': contentType, 'Access-Control-Allow-Origin': '*' }, body };
}

exports.handler = async (event) => {
    const path = event.path.replace('/.netlify/functions/api', '') || '/';
    const method = event.httpMethod;
    const serverUrl = `https://${event.headers.host}`;

    if (method === 'OPTIONS') return { statusCode: 200, headers: { 'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Methods': 'GET,POST,DELETE,OPTIONS', 'Access-Control-Allow-Headers': 'Content-Type,x-admin-key,x-hwid,x-player-id,x-executor-token,x-roblox-id,x-place-id,x-job-id' }, body: '' };
    ...
    try {
        ...
        if (path === '/api/executor/register' && method === 'POST') {
            ...
            if (!body.robloxId || !body.placeId || !body.jobId) return json(400, { success: false, error: "Missing fields" });
            const token = crypto.randomBytes(32).toString('hex');
            await db.setToken(token, { ...body, ip: getClientIP(event), created: Date.now() }, 300);
            await logAccess(event, 'REGISTER', true, { robloxId: body.robloxId });
            return json(200, { success: true, token, expiresIn: 300 });
        }
        ...
        if (path === '/api/auth/challenge' && method === 'POST') {
            ...
            if (isNaN(uid) || isNaN(pid)) return json(400, { success: false, error: "Invalid format" });
            const ban = await db.isBanned(body.hwid, getClientIP(event), uid);
            if (ban.blocked) return json(403, { success: false, error: "Banned", reason: ban.reason, banId: ban.banId });
            if (config.ALLOWED_PLACE_IDS.length > 0 && !config.ALLOWED_PLACE_IDS.includes(pid)) return json(403, { success: false, error: "Game not allowed" });
            const id = crypto.randomBytes(16).toString('hex');
            const nums = Array.from({length: 5}, () => Math.floor(Math.random() * 50) + 1);
            const challenge = { id, userId: uid, hwid: body.hwid, placeId: pid, ip: getClientIP(event), puzzle: { numbers: nums, operation: 'sum' }, answer: nums.reduce((a, b) => a + b, 0) };
            await db.setChallenge(id, challenge, 60);
            await logAccess(event, 'CHALLENGE', true, { id, userId: uid });
            return json(200, { success: true, challengeId: id, puzzle: challenge.puzzle, expiresIn: 60 });
        }
        ...
        if (path === '/api/auth/verify' && method === 'POST') {
            ...
            if (!challenge) return json(403, { success: false, error: "Expired" });
            if (challenge.ip !== getClientIP(event)) return json(403, { success: false, error: "IP mismatch" });
            if (parseInt(body.solution) !== challenge.answer) return json(403, { success: false, error: "Wrong" });
            await db.deleteChallenge(body.challengeId);
            const script = await getScript();
            if (!script) return json(500, { success: false, error: "Not configured" });
            const wrapped = wrapScript(script, serverUrl);
            const isObf = config.SCRIPT_ALREADY_OBFUSCATED || isObfuscated(script);
            if (isObf) {
                await logAccess(event, 'SCRIPT_RAW', true, { userId: challenge.userId });
                return json(200, { success: true, mode: 'raw', script: wrapped, ownerIds: config.OWNER_USER_IDS, whitelistIds: config.WHITELIST_USER_IDS, banEndpoint: `${serverUrl}/api/ban`, meta: { userId: challenge.userId, placeId: challenge.placeId, timestamp: Date.now() } });
            }
            ...
            await logAccess(event, 'SCRIPT_ENC', true, { userId: challenge.userId });
            return json(200, { success: true, mode: 'encrypted', key, chunks, checksum: crypto.createHash('md5').update(wrapped).digest('hex'), ownerIds: config.OWNER_USER_IDS, whitelistIds: config.WHITELIST_USER_IDS, banEndpoint: `${serverUrl}/api/ban`, meta: { userId: challenge.userId, placeId: challenge.placeId, timestamp: Date.now() } });
        }

        if (path === '/api/ban' && method === 'POST') {
            const body = JSON.parse(event.body || '{}');
            if (!body.hwid && !body.playerId) return json(400, { error: "Missing id" });
            const banId = crypto.randomBytes(8).toString('hex').toUpperCase();
            const data = { hwid: body.hwid, ip: getClientIP(event), playerId: body.playerId, playerName: body.playerName, reason: body.reason || 'Auto', toolsDetected: body.toolsDetected || [], banId, ts: new Date().toISOString() };
            if (body.hwid) await db.addBan(body.hwid, data);
            if (body.playerId) await db.addBan(String(body.playerId), data);
            await logAccess(event, 'BAN', true, { playerId: body.playerId, reason: body.reason, banId });
            return json(200, { success: true, banId });
        }

        if (path === '/api/admin/stats' && method === 'GET') {
            const key = event.headers['x-admin-key'] || event.queryStringParameters?.key;
            if (!key || !secureCompare(key, config.ADMIN_KEY)) return json(403, { error: "Invalid key" });
            const stats = await db.getStats();
            return json(200, { success: true, stats, config: { owners: config.OWNER_USER_IDS.length, whitelist: config.WHITELIST_USER_IDS.length } });
        }
        ...
        if (path === '/api/admin/bans' && method === 'GET') {
            ...
            return json(200, { success: true, count: bans.length, bans });
        }
        ...
        if (path.startsWith('/api/admin/bans/') && method === 'DELETE') {
            ...
            if (found) { await db.removeBan(found.key); return json(200, { success: true }); }
        ...
        if (path === '/api/admin/bans/clear' && method === 'POST') {
            ...
            return json(200, { success: true, cleared: count });
        }
